...
#Region "Properties"
    ...
    Dim WorkFileName As String = MS_Edit.WorkFileName
    Dim WorkPath As String = MS_Edit.WorkPath
    ...
    Public Structure StructMapSearch
        ...
        End Property
    End Structure
    Public Structure StructCounter
        Public cStart As Integer
        Public cStop As Integer
        Private _list As List(Of String)
        Public ReadOnly Property Getlist
            Get
                Return _list
...
#Region "Position Functions"
    Private Function MoveNW(ByRef Coords As String, ByRef Spaces As String) As String
        Dim cCords() As String = Coords.Split(",")
        Dim X As Integer = cCords(0).ToInteger
        Dim Y As Integer = cCords(1).ToInteger
        For I = 0 To Spaces - 1
            If IsOdd(Y) Then
                X -= 2
                Y -= 1
    ...
    Private Function MoveNE(ByRef Coords As String, ByRef Spaces As String) As String
        Dim cCords() As String = Coords.Split(",")
        Dim X As Integer = cCords(0).ToInteger
        Dim Y As Integer = cCords(1).ToInteger
        For I = 0 To Spaces - 1
    ...
    Private Function MoveSE(ByRef Coords As String, ByRef Spaces As String) As String
        Dim cCords() As String = Coords.Split(",")
        Dim X As Integer = cCords(0).ToInteger
        Dim Y As Integer = cCords(1).ToInteger
        For I = 0 To Spaces - 1
    ...
    Private Function MoveSW(ByRef Coords As String, ByRef Spaces As String) As String
        Dim cCords() As String = Coords.Split(",")
        Dim X As Integer = cCords(0).ToInteger
        Dim Y As Integer = cCords(1).ToInteger
        For I = 0 To Spaces - 1
        ...
        If selecter2.SelectedIndex <> selecter2.Items.Count() Then
            ...
            Select Case t
                ...
                Case "counter"
                    ...
                    NumStop.Visible = True
                    If DefaultStr <> "" Then

                        Dim num() As String = DefaultStr.Split("-")
                        NumStart.Value = num(0).ToInteger
                        NumStop.Value = num(1).ToInteger

                    ElseIf wVariables.ContainsKey(n) Then
                        NumStart.Value = wVariables.Item(n).cStart
                        NumStop.Value = wVariables.Item(n).cStop
                    Else
                        NumStart.Value = 0
                        NumStop.Value = 0
                    End If
        ...
        For I = 0 To NumericUpDown1.Value - 1
            For t = 1 To wVariables.Count
                ...
                ElseIf type Is GetType(StructMapSearch) Then
                    ...
                    Do While wVariables.Item(t).Getlist.count <= NumericUpDown1.Value
                ...
                ElseIf type Is GetType(StructCounter) Then
                    Dim cList As List(Of String) = New List(Of String)
                    Do While wVariables.Item(t).Getlist.count <= NumericUpDown1.Value
                        Dim tlist As List(Of String) = New List(Of String)
                        For s As Integer = wVariables.Item(t).cStart To wVariables.Item(t).cStop
                            tlist.Add(s.ToString)
                        Next
                        If cList.Count <= NumericUpDown1.Value Then
                            cList.AddRange(tlist)
                        End If
                        If wVariables.Item(t).Getlist.count >= NumericUpDown1.Value Then
                            wVariables.Item(t).setlist.AddRange(cList)
                        End If
                    Loop
                    Str = wVariables.Item(t).getlist(t)
                End If
    ...
    Private Sub ProcessIterations(ByRef Values As List(Of List(Of String)))
        ...
        For i As Integer = 0 To NumericUpDown1.Value - 1
            ...
            For t = 0 To Values(i).Count - 1
                ...
                Dim m As MatchCollection = Regex.Matches(template, "\^(\d+)(nw|ne|sw|se)(\d+)\^", RegexOptions.IgnoreCase)
                For Each s In m
                    Dim spaces As Integer = 0

                    Select Case s.groups(2).ToLower
                        Case "nw"
                            spaces = s.groups(3)
                            template = Regex.Replace(template, "\^" & s.groups(0) & "\^", MoveNW(str, spaces), RegexOptions.IgnoreCase)

                        Case "ne"
                            spaces = s.groups(3)
                            template = Regex.Replace(template, "\^" & s.groups(0) & "\^", MoveNE(str, spaces), RegexOptions.IgnoreCase)

                        Case "se"
                            spaces = s.groups(3)
                            template = Regex.Replace(template, "\^" & s.groups(0) & "\^", MoveSE(str, spaces), RegexOptions.IgnoreCase)

                        Case "sw"
                            spaces = s.groups(3)
                            template = Regex.Replace(template, "\^" & s.groups(0) & "\^", MoveSW(str, spaces), RegexOptions.IgnoreCase)

                        Case Else
                    End Select
                Next
                'm = Regex.Matches(template, "\^(\d+)(nw|ne|sw|se)(\d+)(nw|ne|sw|se)(\d+)\^", RegexOptions.IgnoreCase)
                'For Each s In m
